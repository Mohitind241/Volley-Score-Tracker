...
function addPoint(team, action) {
    ...
    document.getElementById(`team${team}${action}s`).textContent = gameState[teamKey].stats[action];
...
function updateLogDisplay() {
    ...
    logContainer.innerHTML = gameState.gameLog.map(entry => {
        let icon = '';
        let iconColor = '';
        ...
        return `
            <div class="log-item">
                <div class="log-action" style="color: ${iconColor}">
                    ${icon} ${entry.action}
                </div>
                <div class="log-team">${entry.team}</div>
...
function undoLastAction() {
    ...
    if (lastAction.type === 'point') {
        ...
        document.getElementById(`team${teamLetter}${lastAction.action}s`).textContent = gameState[team].stats[lastAction.action];
        updateCurrentScore();
        updateInfoCards();
...
function updateStatsDisplay() {
    ['A', 'B'].forEach(team => {
        ['Spike', 'Block', 'Serve', 'Attack'].forEach(action => {
            document.getElementById(`team${team}${action}s`).textContent = 
                gameState[`team${team}`].stats[action];
        });
    });
}
